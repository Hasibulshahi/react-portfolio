...
function Header() {
  ...
  const navLinkClass = ({ isActive }) =>
    `px-2 py-1 text-sm md:text-base font-medium transition-colors duration-200 ${
      isActive
        ? "text-sky-600"
        : "text-dark-heading dark:text-light-heading hover:text-sky-600"
    }`;

  const socialLinkClass =
    "group inline-flex items-center justify-center w-9 h-9 rounded-md border border-slate-200 dark:border-dark-card transition-colors duration-200 hover:border-sky-500";

  return (
    <header className="py-3 border-b border-slate-200 dark:border-dark-card">
      <div className="container mx-auto max-width">
        <nav className="px-1 md:px-2 py-2">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <NavLink
              to="/"
              className="text-center md:text-left text-base md:text-lg font-semibold text-dark-heading dark:text-light-heading"
            >
              Md Hasibul Hassan
            </NavLink>
            ...
            <ul className="flex flex-wrap items-center justify-center gap-3 md:gap-4">
              {/* <li>
                <NavLink to="/" onClick={toggleClass} className={navLinkClass}>
                  Home
                </NavLink>
              </li> */}
              <li>
                <NavLink to="/about" onClick={toggleClass} className={navLinkClass}>
                  About
                </NavLink>
              </li>
              <li>
              ...
              <li>
                <NavLink to="/contact" onClick={toggleClass} className={navLinkClass}>
                  Contact
                </NavLink>
              </li>
            </ul>
            ...
            <ul className="flex items-center justify-center gap-2">
              <li>
                ...
                >
                  <svg
                    className="fill-dark-heading dark:fill-light-heading group-hover:fill-sky-600 transition-colors duration-200"
                    width="20"
                    height="20"
                    viewBox="0 0 30 30"
                    fill="none"
